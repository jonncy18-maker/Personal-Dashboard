...
- [ ] **Run `npm run migrate` after the Health › Diet PR merges** — migration 028 (`028_health_diet.sql`): `health_profile`, `health_weight_readings`, `health_intake_entries`. Until it is applied, `/health/diet` and `/api/health/*` error; Home is deliberately unaffected (`/api/home-summary` reads the health tables in a guarded helper outside its main query batch, the same shield added for maintenance in #101, precisely so this window can't repeat the #29 outage).
