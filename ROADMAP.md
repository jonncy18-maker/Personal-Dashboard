...
- [ ] Confirm Vercel API token scope is read-only when provisioning — optional now (see 2026-07-14 entry below): unblocks the live Ready/Building/Failed status pill only, no longer required for the card's link to appear at all. **2026-07-17: this is a manual action in the Vercel token UI (scope the token read-only there) — there's no code change and the scope isn't inspectable from the app, so it stays open until John verifies it in the dashboard.**
...
**Vercel token read-only scope (item left open).** John asked to action this, but it's a manual setting in the Vercel token UI, not a code change — and the token's scope isn't inspectable from the app. Left the to-do open with a note; John verifies it in the dashboard. Only affects the live deploy-status pill, nothing else.
