...
*(Not dated history — live items that outlast a single session. Remove when done.)*
...
- **Tier 1 — sender rules:** clicking "X" on an email creates a structured, deterministic sender-hide rule (e.g. "hide all future emails from dominos.com"). After creation, filtering is a plain database check. *(Refined 2026-07-13: no AI needed — sender is parsed directly from the header.)*
- **Tier 2 — content rules:** for senders John generally wants but sometimes doesn't (e.g. his bank sending both statements and promotional content). A "manage sender" action lets him type a plain-language rule; evaluated per future email from that sender. *(Refined 2026-07-13: Haiku used only for the semantic residual Gmail's native categories can't express.)*
...
- **AI Projects domain:** sources live status from the **Vercel API** (`list_projects`, `get_project`, `list_deployments`) — not GitHub as a commit/issue tracker. *(Note: GitHub re-entered later for "Next Up" parsing — a different purpose.)*
...
- **Stack:** Next.js + Vercel + new standalone Neon project, built to the `STACK_BLUEPRINT.md` reference patterns *(auth later dropped — see 2026-07-13)*
