...
- [ ] **No migration-runner — a schema-changing PR doesn't apply itself.** Surfaced 2026-07-16 by the Travel redesign (#29): the migration file (`005_travel_map_brief.sql`) merged and deployed cleanly, but the live Neon DB was never updated, so `/api/trips` 500'd ("Could not load trips.") until the migration was applied by hand via the Neon MCP. `neon/migrations/*.sql` are written and merged but nothing runs them against the actual database — that's always been a manual step (per CLAUDE.md §6's "no ORM, no migration framework" convention), it just hadn't bitten yet because earlier migrations either predated real usage or were applied proactively in the same session. Worth deciding: keep it fully manual but make the PR checklist say so explicitly ("did you apply migration NNN to Neon?"), or add a lightweight apply-on-deploy step. Not fixed here — just a real gap to close before it causes a live outage instead of a preview one.
...
- [ ] **Wishlist trip status** — _small, deferred._ The ChatGPT mock's Trip Collection had an Upcoming/Past/**Wishlist** tab. Wishlist needs a new value in the `trips.status` CHECK (a migration) + a way to set it. Left out of the redesign; pick up when the tabbed collection is wanted.
