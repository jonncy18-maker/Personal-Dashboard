...
**Still unverified.** Whether each skill actually *fires* on the intended files in practice — only observable across real sessions. Worth watching over the next few: if a domain's rules aren't showing up when its files are touched, the `description` needs tightening.
