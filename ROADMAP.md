...
- [x] **Run `npm run migrate` after the Health › Diet PR merges** — 2026-09-16, applied via the Neon MCP right after #114 merged. Migration 028 (`028_health_diet.sql`) created `health_profile`, `health_weight_readings` and `health_intake_entries`; confirmed all three exist, the three `updated_at` triggers are attached, the singleton profile row is present, `schema_migrations` records the file, and the `source` CHECK (`label`/`recall`/`estimated`) is enforced at the database — so the tilde rule has a floor under it even if a future code path forgets to validate. _(Applied statement-by-statement rather than via the runner, same as prior entries — the net state is identical to an `npm run migrate` run.)_ Original note: — migration 028: until it is applied, `/health/diet` and `/api/health/*` error; Home is deliberately unaffected (`/api/home-summary` reads the health tables in a guarded helper outside its main query batch, the same shield added for maintenance in #101, precisely so this window can't repeat the #29 outage).
- [ ] **Provision `HEALTH_MCP_TOKEN` in Vercel (Production _and_ Preview)** — the bearer token gating the Health MCP server's write tools. It **fails closed**: until it is set, `/api/mcp/health` returns 503 and Claude cannot log food or weight. Then add the MCP server to Claude with `Authorization: Bearer <token>` against `https://<app>/api/mcp/health`.
- [ ] **Enter the Health profile once** — sex, height, age (or date of birth), activity multiplier, goal weight and goal date. Until then `/health/diet` honestly shows "no target yet" with the missing inputs named, rather than a plausible default.
