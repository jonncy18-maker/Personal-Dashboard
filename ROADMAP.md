...
## 2026-08-09 (cont'd 3) — App-wide AI Assistant (sixth AI use — the agentic one, on Opus)

John: "add almost an AI Assistant component to the entire app… it should use the Anthropic key and be able to do anything the user can do manually." Built as a floating chat panel on every page (`components/AssistantPanel.jsx`, mounted in `AppShell` next to the SW registration) → `POST /api/assistant` → `lib/assistant.js`, a Claude Opus (`claude-opus-5`) tool-use loop (max 10 tool rounds per message, `maxDuration = 300`).
...
Model: **Opus, not Haiku — a deliberate divergence from the app's Haiku-only rule**, documented in CLAUDE.md §7. The five narrow uses stay pinned to Haiku; an agentic assistant juggling a 35-tool catalog is a different shape of problem. `ASSISTANT_MODEL` is one constant if the cost/quality tradeoff should change. The system prompt (cached via `cache_control`) carries the app's house rules: Gmail read-only, ideas-vs-schedules boundary = due date, confirm-before-delete, never fabricate data. Bumped `@anthropic-ai/sdk` 0.30 → 0.116 (the 0.30-era SDK predates prompt-caching GA; `messages.create` call sites unchanged).
