...
import { getCalendarRenameMap, renamedTitle } from './calendar-events';
...
async function findCalendarMatch() {
  ...
  try {
    ...
    // Matching runs on the raw event (summary/organizer/etc), but the title
    // we surface must honor a local rename — otherwise renaming the Spanish
    // call on /calendar leaves the Up Next / Language card showing the raw
    // Google title (e.g. "John Shaw"). Apply the same override the /calendar
    // view uses, keyed by this occurrence's id or its series id.
    const renameMap = await getCalendarRenameMap();
    const title = renamedTitle(renameMap, {
      id: match.id,
      recurringEventId: match.recurringEventId,
