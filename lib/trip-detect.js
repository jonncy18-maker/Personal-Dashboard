...
export async function detectTripFromEmail({ subject, body }) {
  ...
  try {
    ...
    const res = await anthropic.messages.create({
      ...
      max_tokens: 200,
      system:
        'You decide whether an email evidences a specific real trip the reader has booked ' +
        '(a flight, hotel, cruise, or tour) and extract its basics. ' +
        ...
        "A PROMOTIONAL email can still evidence the reader's OWN booking: cruise lines and " +
        'airlines routinely wrap a real reservation block (ship or flight, sail/departure date, ' +
        'itinerary name, booking number) inside a sale email. When such a block names a ' +
        'specific booked departure, that IS a real trip — set is_trip=true and extract it, ' +
        'ignoring the advertising around it. ' +
        'Set is_trip=false only when the email merely advertises trips the reader COULD book, ' +
        "or is a price alert or generic newsletter with no booking of the reader's own. " +
        'If the email gives a start date and a length ("7 Night"), derive end_date from those ' +
        'two — that is arithmetic on stated facts, not invention. ' +
        'Be conservative: when unsure, low confidence.',
      messages: [
        {
          role: 'user',
