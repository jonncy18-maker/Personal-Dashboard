...
// same Nominatim /search endpoint as geocodeQueryResult, but limit>1 so the
// UI can show a picklist instead of silently taking the first match. Each
// suggestion already carries validated coords, so picking one skips a
// second geocode at save time. Fails soft to an empty list — same
// discipline as every other geocode call in this app.
export async function searchPlaces(query, limit = 5) {
  ...
  try {
    const res = await fetch(
      `https://nominatim.openstreetmap.org/search?format=json&limit=${encodeURIComponent(
        limit
    ...
    if (!res.ok) {
      ...
      // client's side — log the real cause (rate-limited, blocked, etc.)
      // so it shows up in Vercel runtime logs instead of vanishing silently.
      console.error(`searchPlaces: Nominatim returned ${res.status} for %o`, q);
      return [];
    }
    ...
    if (data.length === 0) {
      console.warn(`searchPlaces: Nominatim found no matches for %o`, q);
    }
