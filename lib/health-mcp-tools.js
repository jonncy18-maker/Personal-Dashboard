...
export const HEALTH_TOOLS = [
  {
    ...
    description:
      ...
      "the day's vegetable baseline under-counts.",
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description:
      ...
      'and water drunk (water_oz_total, water_cups_total, water_entries, and ' +
...
async function readDay(sql, dateStr) {
  ...
  const water = waterProgress(waterEntries, profile?.water_target_oz ?? null);
  ...
  if (name === 'log_food') {
    ...
    const [row] = await sql`
      ...
      VALUES (${entryDate}, ${args.meal}, ${description}, ${Math.round(calories)},
              ...
              ${veggies.value}, ${args.source}, ${args.source_detail || null},
              'mcp')
      RETURNING id
    `;
    const day = await readDay(sql, entryDate);
  ...
  if (name === 'list_favorite_meals') {
    const rows = await sql`
      SELECT id, name, meal, description, calories, protein_g, carbs_g,
             fat_g, veggie_servings, source, source_detail
  ...
  if (name === 'save_favorite_meal') {
    ...
    const [row] = await sql`
      ...
      VALUES (${favName}, ${args.meal || null}, ${description},
              ...
              ${macros.fat_g}, ${veggies.value}, ${args.source},
  ...
  if (name === 'log_favorite_meal') {
    ...
    const [row] = await sql`
      ...
      VALUES (${entryDate}, ${meal}, ${favorite.description}, ${favorite.calories},
              ...
              ${favorite.veggie_servings}, ${favorite.source},
