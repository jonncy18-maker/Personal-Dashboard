...
import { deviceToday } from './device-time';
...
export async function callHealthTool(name, args) {
  ...
  if (name === 'log_water') {
    ...
    const entryDate = args.entry_date || (await deviceToday({ sql }));
    const [row] = await sql`
  ...
  if (name === 'get_day') {
    const dateStr = args.date || (await deviceToday({ sql }));
  ...
  if (name === 'log_weight') {
    ...
    const readingDate = args.reading_date || (await deviceToday({ sql }));
    await sql`
  ...
  if (name === 'add_recommended_meal') {
    ...
    const forDate =
      args.horizon === 'today'
        ? args.for_date || (await deviceToday({ sql }))
        : null;
  ...
  if (name === 'update_recommended_meal') {
    ...
    if ('for_date' in args || 'horizon' in args) {
      forDate =
        horizon === 'today'
          ? args.for_date ||
            dateOnly(current.for_date) ||
            (await deviceToday({ sql }))
          : null;
    } else {
  ...
  if (name === 'update_health_profile') {
    ...
    const [row] = await sql`
      ...
      RETURNING *
    `;
    const day = await readDay(sql, await deviceToday({ sql }));
    return toolText(
  ...
  if (name === 'get_trailing_net_calories') {
    ...
    const range = ['week', 'month', 'ytd'].includes(args.range)
      ? args.range
      : 'week';
    const todayStr = await deviceToday({ sql });
    const from =
      range === 'ytd'
