...
// The vehicle's display name, from the profile on mileage_settings — one car
// for the whole Car domain, so both tabs render it from this one function.
// Until 2026-09-13 the Mileage hero hardcoded "Tesla" / "Model 3" in JSX,
// which made the app look like it already knew the car while the columns were
// empty — and made Maintenance's separate vehicle form read as re-entering
// something already stored. Returns null when unset so callers show a real
// prompt instead of asserting a car nobody entered.
export function vehicleLabel(settings) {
  const parts = [
...
export function hasVehicle(settings) {
  return !!settings?.vehicle_model;
}
...
// A one-time scenario's mileage once realization (migration 034) is taken
// into account — the same number the checkpoint/monthly forecast lands, and
// what the scenario card shows John, computed once so the two can never
// drift apart. Without an occurrence_count set, every trip is still
// forecast (today's behavior, unchanged). With one set, only the trips not
// yet marked realized still count.
export function remainingOneTimeMiles(scenario) {
  const total = Number(scenario.one_time_miles) || 0;
  const count = Number(scenario.occurrence_count) || 0;
  if (!count) return total;
  const realized = Math.min(
    Math.max(Number(scenario.realized_count) || 0, 0),
    count
  );
  return Math.round((total * (count - realized)) / count);
}

export function scenarioImpactForCheckpoint({
  scenario,
  leaseStart,
  checkpointDate,
  n,
  ...
  if (scenario.occurrence === 'one_time') {
    ...
    return remainingOneTimeMiles(scenario);
  }
...
export function scenarioImpactAt({ scenario, leaseStart, targetDate }) {
  ...
  if (scenario.occurrence === 'one_time') {
    ...
    return parseDateInput(targetDate) < parseDateInput(landsBy)
      ? 0
      : remainingOneTimeMiles(scenario);
  }
  ...
  // A scenario with a later effective_start is zero until it starts, then
  // ramps to the (already prorated) checkpoint figure. Interpolating from
  // lease start instead leaked miles into the months before it began (Codex
  // audit F07). Every checkpoint mark at or before the start is already 0,
  // so adding the start as its own 0-impact mark keeps the checkpoints
  // themselves unchanged.
  const effectiveDays = scenario.effective_start
    ? daysBetween(
        parseDateInput(leaseStart),
        parseDateInput(scenario.effective_start)
      )
    : 0;
  ...
  if (effectiveDays > 0) {
    marks.push({ days: effectiveDays, impact: 0 });
    marks.sort((a, b) => a.days - b.days);
  }
  for (let i = 1; i < marks.length; i++) {
    if (targetDays <= marks[i].days) {
      ...
      const b = marks[i];
      const t =
        b.days === a.days ? 0 : (targetDays - a.days) / (b.days - a.days);
...
// Miles an accepted Travel Day Exclusion takes off a projection anchored at
// a real odometer reading. The anchor reading already reflects every trip
// day on or before it (the driving that didn't happen is simply missing
// from the odometer), so only the trip's days AFTER the anchor still need
// subtracting from the pace line: a trip fully before the anchor counts 0,
// one straddling it counts the after-anchor share of its days, and one
// fully after counts in full. Subtracting the whole figure regardless
// double-counted past trips and could push a forecast below the real
// odometer (Codex audit F06). Still nothing until the trip has ended by
// `targetDate`, as before.
export function exclusionMilesAt({ exclusions, anchorDate, targetDate }) {
  const anchor = parseDateInput(anchorDate);
  const target = parseDateInput(targetDate);
  return (exclusions || []).reduce((sum, e) => {
    const end = parseDateInput(e.end_date);
    if (end > target) return sum;
    const miles = Number(e.miles_excluded) || 0;
    const totalDays = tripDayCount(e.start_date, e.end_date);
    const afterAnchor = Math.min(totalDays, daysBetween(anchor, end));
    if (afterAnchor <= 0 || totalDays <= 0) return sum;
    return sum + (miles * afterAnchor) / totalDays;
  }, 0);
}
...
export function projectAt({
  ...
  const exclusionSubtract = Math.round(
    exclusionMilesAt({ exclusions, anchorDate, targetDate })
  );
...
// by yet) — and only its days after the anchor reading, which already
// reflects the rest (see exclusionMilesAt). A null pace (no readings logged yet) projects as "unknown"
...
export function projectCheckpoint({
  ...
  const exclusionSubtract = Math.round(
    exclusionMilesAt({ exclusions, anchorDate, targetDate: date })
  );
