...
// A trip's `status` is a manually-set field — it never auto-transitions when
// a trip's dates pass, so an 'upcoming' trip can go stale. Derive "has this
// already happened" from the actual dates instead of trusting the stored
// status. Shared by the Travel list (hero/timeline/past split) and the trip
// detail page (recap vs. edit view), so the two never disagree about which
// bucket a trip falls into.
export function isTripPastByDate(trip, now = new Date()) {
  const ref = trip.end_date || trip.start_date;
  if (!ref) return false;
  return startOfDay(ref) < startOfDay(now);
}

// The Past-travels bucket itself: an explicit 'past' status, or an 'upcoming'
// trip whose dates have already elapsed. Wishlist trips are never past,
// regardless of dates — they're aspirational, not booked travel.
export function isPastTrip(trip, now = new Date()) {
  if (trip.status === 'wishlist') return false;
  return trip.status === 'past' || isTripPastByDate(trip, now);
}
