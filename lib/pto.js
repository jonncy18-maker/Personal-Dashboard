...
export function ptoSummary({
  ...
  const counted = (trips || [])
    ...
    // A trip with zero date overlap with `year` doesn't belong in that
    // year's list at all — filtering here (rather than after computing
    // tripCountedDays) also keeps a pto_days_override from counting toward
    // a year the trip never touches, since the override branch below
    // doesn't itself check for year overlap.
    .filter((t) => clampToYear(t.start_date, t.end_date, year) != null)
    .map((t) => ({
      trip: t,
  ...
  return {
    ...
    trips: counted.map(({ trip, autoDays, counted: c }) => ({
      ...
      counted: c,
      // Present when the caller passed already-collapsed trips (see
      // lib/trip-merge.js's collapseMergedTrips) — lets the panel show "this
      // is a merged group of N legs" instead of a plain trip row.
      isMerged: !!trip.is_merged,
      legs: trip.legs || [],
    })),
...
// ─── net figure — an explicit, opt-in blend, never automatic ───────────────
// `left` and the banked ledger stay computed and stored fully separately
// (ptoSummary above never touches this); this is purely a display-time sum,
// gated by pto_settings.use_banked_for_shortfall (migration 024) so a
// combined headline only ever appears because John turned it on from the
// Planning sub-tab — never silently.
export function netPtoLeft(left, bankedAvailable, useBankedForShortfall) {
  return useBankedForShortfall ? left + bankedAvailable : left;
}
