...
export async function parseItineraryFromEmail({
  ...
  const res = await anthropic.messages.create({
    ...
    system:
      ...
      'fences. Each element is {"date": "YYYY-MM-DD" or "", "title": string, "location": ' +
      'string, "leg": string, "notes": string}. One element per distinct day, port of call, ' +
      'flight segment, or hotel night, in chronological order. Use "" for date when no real ' +
      'calendar date is clearly present — never invent one. Keep title short (e.g. "Juneau — ' +
      'whale watching", "Flight LAX→SEA"). For "location", give the real city/port that stop ' +
      'happens in, as a geocodable place name ("Cartagena, Colombia", "Juneau, Alaska") — use ' +
      '"" for at-sea days, layovers, or anything with no single place. For "leg", give a short ' +
      'group name only when the trip has distinct segments (e.g. a country or cruise name like ' +
      '"Taiwan" or "Japan cruise"); otherwise "". Put times, confirmation numbers, and other ' +
      'detail in notes. If there is no itinerary content, respond with [].',
  ...
  try {
    ...
    return parsed
      .map((d) => ({
        ...
        leg: typeof d?.leg === 'string' ? d.leg : '',
        notes: typeof d?.notes === 'string' ? d.notes : '',
      }))
      .filter((d) => d.title || d.notes || d.date || d.location);
  } catch {
    return [];
  }
}
