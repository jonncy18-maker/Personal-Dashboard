import { dateOnly } from './db';

// Pure aggregation for the Travel Stats bar. Every number here comes straight
// from real trip rows — no fabricated totals (CLAUDE.md's hard rule). Points/
// miles are deliberately absent: they'd need a loyalty-account integration with
// no data source, so they're left out rather than invented.
//
// "Real trips" = status upcoming or past. Wishlist trips are aspirational, not
// travel that happened or is booked, so they're excluded from every tile. The
// framing is whole-log (past + upcoming), John's call — not past-only.

const REAL_STATUSES = new Set(['upcoming', 'past']);

// A cruise is detected by the word "cruise" in the destination or notes — the
// same signal lib/destination.js already treats as a generic trip-type word.
// Transparent heuristic, not a stored flag; there's no cruise column to key off.
export function isCruise(trip) {
  const hay = `${trip?.destination || ''} ${trip?.notes || ''}`.toLowerCase();
  return /\bcruise\b/.test(hay);
}

// Whole nights for a trip = end_date − start_date in days. Both are date-only
// columns (UTC-midnight instants), so compare them as day strings to avoid the
// negative-offset off-by-one that bit date displays before (ROADMAP 2026-07-15).
export function tripNights(trip) {
  const start = dateOnly(trip?.start_date);
  const end = dateOnly(trip?.end_date);
  if (!start || !end) return 0;
  const ms = Date.parse(`${end}T00:00:00Z`) - Date.parse(`${start}T00:00:00Z`);
  if (!Number.isFinite(ms)) return 0;
  const nights = Math.round(ms / 86400000);
  return nights > 0 ? nights : 0;
}

// `rows` should already be collapsed via lib/trip-merge.js's
// collapseMergedTrips() before reaching here, so a merged journey (several
// leg rows folded into one root) counts as ONE trip, not one per leg — the
// same anti-duplication rule that applies to PTO. A collapsed root's own
// start_date/end_date already span the full merged range, so `nights` still
// totals correctly with no extra work; countries still credit every distinct
// country actually visited by pulling each leg's own resolved country in
// alongside the root's.
// Cruise nights for one collapsed trip, each cruise segment counted once on
// its own dates. A merged root's start/end are the WHOLE journey's range, so
// using them for a cruise root counted hotel and flight legs as cruise nights
// (Codex audit F09: a 7-night cruise + 2-night hotel read as 9); segments are
// unioned so a cruise leg overlapping a cruise root isn't counted twice.
function cruiseNightsFor(trip) {
  const segments = [];
  if (isCruise(trip)) {
    segments.push({
      start_date: trip.own_start_date ?? trip.start_date,
      end_date: trip.own_end_date ?? trip.end_date,
    });
  }
  for (const leg of trip.legs || []) {
    if (isCruise(leg)) segments.push(leg);
  }
  const ranges = segments
    .map((seg) => [dateOnly(seg.start_date), dateOnly(seg.end_date)])
    .filter(([a, b]) => a && b && a < b)
    .sort((x, y) => (x[0] < y[0] ? -1 : x[0] > y[0] ? 1 : 0));
  let nights = 0;
  let curStart = null;
  let curEnd = null;
  for (const [a, b] of ranges) {
    if (curEnd != null && a <= curEnd) {
      if (b > curEnd) curEnd = b;
      continue;
    }
    if (curEnd != null)
      nights += tripNights({ start_date: curStart, end_date: curEnd });
    curStart = a;
    curEnd = b;
  }
  if (curEnd != null)
    nights += tripNights({ start_date: curStart, end_date: curEnd });
  return nights;
}

export function computeTravelStats(rows) {
  const real = (rows || []).filter((r) => REAL_STATUSES.has(r.status));

  const countries = new Set();
  let nights = 0;
  let cruiseNights = 0;

  for (const trip of real) {
    const n = tripNights(trip);
    nights += n;
    cruiseNights += cruiseNightsFor(trip);
    if (trip.country) countries.add(trip.country.trim().toLowerCase());
    for (const leg of trip.legs || []) {
      if (leg.country) countries.add(leg.country.trim().toLowerCase());
    }
  }

  return {
    trips: real.length,
    nights,
    countries: countries.size,
    cruiseNights,
  };
}
