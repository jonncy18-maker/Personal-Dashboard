...
// Gmail's per-user rate limit ("Quota exceeded ... Units per minute per
// user") — told apart from a genuinely bad/exhausted query. Once this fires,
// EVERY remaining query in the same run fails too (the minute's budget is
// gone), so a caller must stop and preserve its cursor rather than treating
// each failure as "this query is done, advance" — see app/api/
// trip-history-scan, where advancing on this by mistake cascaded query_index
// to the end and falsely marked a 10-year scan "done" after a rate limit.
export function isQuotaError(err) {
  const message = String(err?.message || '');
  const status = err?.response?.status ?? err?.code;
  return message.includes('Quota exceeded') || status === 429;
}
