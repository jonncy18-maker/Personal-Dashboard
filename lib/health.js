...
// The day's water against the optional goal. No goal → `met: null`, never a
// verdict against a number John didn't set.
export function waterProgress(entries, targetOz) {
  const total =
    Math.round(
      (entries || []).reduce((sum, e) => sum + (Number(e.ounces) || 0), 0) * 10
    ) / 10;
