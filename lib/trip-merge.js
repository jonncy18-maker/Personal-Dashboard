...
export function collapseMergedTrips(trips) {
  ...
  return list
    ...
    .map((root) => {
      ...
      // own_* keep the root row's own dates: the widened range is right for
      // whole-journey nights and PTO, but a per-segment count (cruise nights)
      // needs what the root itself covers.
      return {
        ...root,
        ...range,
        own_start_date: root.start_date,
        own_end_date: root.end_date,
        legs,
        is_merged: true,
      };
    });
