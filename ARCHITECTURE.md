...
| Car         | `/car`         | Neon (`mileage_settings`, `mileage_readings`, `mileage_trips`, `mileage_scenarios`, `maintenance_items`, `maintenance_records`) + Google Geocoding API (geocode) + OSRM (driving distance, still free/keyless) — one lookup per trip logged. Maintenance due dates invert the mileage forecast; no second pace calculation. `/mileage` redirects to `/car/mileage` (installed-PWA start URL) | No                           |
| Health      | `/health/diet` | Neon (`health_profile`, `health_weight_readings`, `health_intake_entries`). Writes arrive primarily from Claude via the MCP server at `/api/mcp/health` (bearer-token gated), not from the browser                                                                                                                                                                                           | No                           |
