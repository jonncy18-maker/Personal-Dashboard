...
A private, single-user planning hub for John. One home page shows a status card per domain; each card links into that domain's full page. Seven domains across eight routes: AI Projects, Travel, Mileage, Schedules, Language Learning, Idea Board, Email. Plus a `/calendar` view — a read-only month view of Google Calendar, a cross-cutting surface rather than a new data domain (it owns no table). Email additionally feeds the Home hero's "To-do's" block: emails John flags become to-do's stored locally in `email_todos` (Gmail stays read-only — no star is written back).
...
| Travel      | `/travel`      | Neon (`trips`) + Gmail (itinerary import). Also hosts the PTO Planner: Neon (`pto_settings`, `pto_holidays`, `pto_entries`, `pto_scenarios`) — pure date math, no external calls | Haiku — import parse only |
| Mileage     | `/mileage`     | Neon (`mileage_settings`, `mileage_readings`, `mileage_trips`, `mileage_scenarios`) + Google Geocoding API (geocode) + OSRM (driving distance, still free/keyless) — one lookup per trip logged  | No                              |
