...
**App-wide refresh signal.** The TopBar refresh button drives `lib/refresh.jsx` (`RefreshProvider`/`useRefresh`, wrapping the shell in `AppShell`): calling `refresh()` bumps a `refreshKey` that `useHomeSummary` and `useResource` re-fetch on. A new data hook should subscribe to `useRefresh()` so the button covers it. Pages that still hand-roll their own `fetch` (Travel, Email, Ideas, Schedules) are **not** wired yet — migrate them onto `useResource` to bring them under the button.
