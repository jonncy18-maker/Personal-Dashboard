...
5. **Vercel → personal-dashboard → Settings → Environment Variables →** update `GOOGLE_REFRESH_TOKEN` for **both Production and Preview** (§4's gotcha), then **redeploy** — env changes do not apply to already-built deployments.
6. Verify: `/travel` → Scan Gmail must NOT say "Gmail access has expired"; `/calendar` shows events again.
