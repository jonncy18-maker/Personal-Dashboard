...
- **The odometer log is the only source of cumulative miles.** `mileage_readings` (dated, one per day, upserted) is what pace and every forecast are computed from. `mileage_trips` (the point-to-point journal, geocoded via `lib/geocode.js` + routed via the free keyless OSRM demo server in `lib/route-distance.js`, no paid tile provider — same rule as Travel's map) is a supplementary log for context; it is **never** summed into the odometer total, since two readings can legitimately disagree with what got logged as trips in between.
