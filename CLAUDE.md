...
| Anthropic API key (Claude Haiku)                       | none (server-only) | Vercel env (Production + Preview)               | Narrow uses only: Email Tier 2 semantic residual, Travel itinerary parse, Travel AI Brief, Language's French hours screenshot import. See §7                                                                                                                                    |
...
- **The catalog covers every domain, Mileage included (added 2026-08-26).** Mileage shipped as its own set of routes without an assistant catalog entry for a while — the "add the underlying route first, then a catalog entry" rule from the bullet above means a new route is only actually usable by the assistant once it's added to `CATALOG` in `lib/assistant.js`; it's easy to ship the route and forget the catalog half. The assistant can read full Mileage state (`get_mileage`), log odometer readings, log/delete trip-journal entries, add/update/delete scenarios (manual or leg-based), add/delete usual-trip legs and favorite places, and set lease settings **and the "usual trips" baseline override** (`update_mileage_settings`'s `usual_miles`/`usual_period`/`usual_active`) — so "build my baseline mileage" is doable through the assistant, not just the UI. Same discipline as everywhere else: `usual_active` and a scenario's `active` flag are never flipped without John asking for that outcome.
